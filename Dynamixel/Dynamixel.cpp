 ...
 *****************************************************************************
 ...
 MX-106, MX-64, MX-28,
 XC330-M288, XC330-M181, XC330-T288, XC330-T181, 
 XW430-T200, XW430-T333, XW430-T210, XW430-T350,
 XD540-T150, XD540-T270 
 XW540-T260, XW540-T140, 
 XH540-V270, XH540-W270, XH540-V150, XH540-W150, XH540-W270 
 XM540-W270, XM540-W150,
 XH430-V350, XH430-W350, XH430-W350, XH430-W210, XH430-V210
 XM430-W350, XM430-W210,
 XL430-W250, 2XL430-W250, 
 XC430-W240, XC430-W250, XC430-W150, 2XC430-W250
...
int DynamixelClass::readRegister(unsigned char registerLenght)
{
	if (protocolVersion == 2){
		...
		while((availableData() < 13) & (timeCounter < TIME_OUT)){  
		...
		int registerValue = -1;
		unsigned int timeCounter = 0;
		while((availableData() < 7 + registerLenght) & (timeCounter < TIME_OUT)){
			timeCounter++;
			delayus(1000);
		}

		while (availableData() > 2 + registerLenght){
			incomingByte = readData();
...
int DynamixelClass::readError(void)
{
	if (protocolVersion == 2){
		...
		return (NO_RESPONSE_ERROR);							// No Response
	} else if (protocolVersion == 1){
		unsigned int timeCounter = 0;
		while((availableData() < 7) & (timeCounter < TIME_OUT)){  
			timeCounter++;									// Wait for Data
			delayus(1000);
		}
...
int DynamixelClass::reset(unsigned char motorID, unsigned char resetMode)
{
	if (protocolVersion == 2){
		...
		while(availableWrite() < STD_HW_BUFFER);delayus(TESTTIME);
...
{   
	if (protocolVersion == 2){
		...
		unsigned char dataPacket[11] = {DNMXL_H1,DNMXL_H2,DNMXL_H3,DNMXL_RSVD,motorID,DNMXL_WRITE6_LENGTH_LOW,DNMXL_WRITE6_LENGTH_HIGH,
									   DNMXL_WRITE,DNMXL_BR_REG_LOW,DNMXL_BR_REG_HIGH,bRate};
    	unsigned short crcBytes = generateCRC(dataPacket,11);
    	unsigned char  crcLowByte = (crcBytes & 0x00FF);
    	unsigned char  crcHighByte = (crcBytes >> 8) & 0x00FF;
		switchCom(directionPin,TX_MODE);
